...
public:
    ...
    FG_eval(Eigen::VectorXd coeffs) { this->coeffs = std::move(coeffs); }
    ...
    void operator()(ADvector &fg, const ADvector &vars) {
        ...
        for (int t = 0; t < N; t++) {
            // Multiplying by 2k to make these values more influential
        ...
        for (int t = 0; t < N - 2; t++) {
            // 200/10 to make steering smoother
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
    ...
    for (int j = 1; j < N; ++j) {
        results.push_back(solution.x[x_start + j]);
        results.push_back(solution.x[y_start + j]);
    }
